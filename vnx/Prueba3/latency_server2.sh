...
for file in "$@"; do
    ...
    latency_values=$(sudo grep 'ms' "$1" | awk -F '[ /]+' '{gsub("/", " ", $15); print $15}' | grep -v '^$')
...
cd test3_results
./vectorization.sh latency_h4 20 60
./vectorization.sh latency_h5 40 40
./vectorization.sh latency_h3 60 20
