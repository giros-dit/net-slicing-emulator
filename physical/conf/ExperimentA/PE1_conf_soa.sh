...
tc filter add dev enp3s0 protocol ip parent ffff:fff1 prio 1 u32 \
    ...
    police rate 32mbit burst 4615b overhead 38 continue

tc filter add dev enp3s0 protocol ip parent ffff:fff1 prio 1 u32 \
    match ip src 10.0.0.2 \
    action pedit ex munge ip dsfield set 0x04 pipe \
    csum ip and udp pipe \
    police rate 68mbit burst 4615b overhead 38 drop

tc filter add dev enp3s0 protocol ip parent ffff:fff1 prio 1 u32 \
    match ip src 10.0.0.4 \
    police rate 4mbit burst 4615b overhead 38 continue

tc filter add dev enp3s0 protocol ip parent ffff:fff1 prio 1 u32 \
    match ip src 10.0.0.4 \
    action pedit ex munge ip dsfield set 0x04 pipe \
    csum ip and udp pipe \
    police rate 96mbit burst 4615b overhead 38 drop

tc filter add dev enp3s0 protocol ip parent ffff:fff1 prio 1 u32 \
    match ip src 10.0.0.6 \
    police rate 52.8mbit burst 4615b overhead 38 continue

tc filter add dev enp3s0 protocol ip parent ffff:fff1 prio 1 u32 \
    match ip src 10.0.0.6 \
    action pedit ex munge ip dsfield set 0x04 pipe \
    csum ip and udp pipe \
    police rate 47.2mbit burst 4615b overhead 38 drop

tc filter add dev enp3s0 protocol ip parent ffff:fff1 prio 1 u32 \
    match ip src 10.0.0.8 \
    police rate 1.2mbit burst 4615b overhead 38 continue

tc filter add dev enp3s0 protocol ip parent ffff:fff1 prio 1 u32 \
    match ip src 10.0.0.8 \
    action pedit ex munge ip dsfield set 0x04 pipe \
    csum ip and udp pipe \
    police rate 98.8mbit burst 4615b overhead 38 drop

tc filter add dev enp3s0 protocol ip parent ffff:fff1 prio 1 u32 \
    match ip src 10.0.0.10 \
    police rate 100mbit burst 4615b overhead 38 drop
