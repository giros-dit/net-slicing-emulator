...
def save_backlog(backlog_list):
    result = "tc -s -d class show dev enp4s0 | grep -E -A 2 'class prio 1:1|class drr'| grep backlog | awk '{{print $3}}' | sed 's/p//' | tr '\n' ',' | sed 's/,$//'" 
...
def save_packetloss(pktloss_list):
    result = "tc -s -d class show dev enp4s0 | grep -E -A 2 'class prio 1:1|class drr' | grep dropped | awk '{{print $7}}' | sed 's/,//g' | tr '\n' ',' | sed 's/,$//'"
...
for i, list in enumerate(backlogs):
    with open(f'/root/clase_{i + 1}_backlog_{parameter}', 'a') as file:
        for v in list:
            file.write(f'{v}\n')

for i, list in enumerate(pktloss):
    with open(f'/root/clase_{i + 1}_pktloss_{parameter}', 'a') as file:
        for v in list:
            file.write(f'{v}\n')
