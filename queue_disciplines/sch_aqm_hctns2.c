...
/*
static bool is_skb_in_queue(struct Qdisc *sch, struct sk_buff *skb) {
	struct sk_buff *iter;
	for (iter = sch->q.head; iter; iter = iter->next) {
		if (iter == skb)
			return true;
	}
	return false;
			 ...
			 struct sk_buff **to_free)
{
	...
	/*	int ret = qdisc_enqueue_tail(skb, sch);
		if (ret == NET_XMIT_SUCCESS && has_ecn) {
			struct ecn_skb_node *node = kmem_cache_alloc(ecn_node_cache, GFP_ATOMIC);
			if (node) {
				node->skb = skb;
	...
	*/
		...
		if (qlen >2) {
			trace_printk("previous to next: %p",  sch->q.head->next->prev);
			}
	...
	*/
	if (q->ecn_count > 0) {
		...
		if (ecn_victim) {
			...
			trace_printk("Delete ECN packet to enqueue non-ECN packet; qlen: %u, ecn_count: %u, in_queue: %s, packet: %p\n",
             sch->q.qlen, q->ecn_count, in_queue ? "true" : "false", ecn_victim);
			int ret = qdisc_enqueue_tail_aqm_hctns(skb, sch); // Enqueue the new packet
			//qdisc_tree_reduce_backlog(sch, 0, prev_backlog - sch->qstats.backlog); // Update the backlog
			trace_printk("Queue length: %u\n", sch->q.qlen);
			spin_unlock_bh(&q->lock); // Unlock the list
			return ret;	
		}
	...
	if (skb && skb_has_ecn_ce(skb) && q->ecn_count > 0) {
		...
		list_for_each_entry_safe(node, tmp, &q->ecn_list, list) {
			if (node->skb == skb) {
				...
				trace_printk("ecn_victim found; ecn_count: %d\n", q->ecn_count);
				break;
			}
		}
		if (!found)
			trace_printk("ecn_victim not found dequeueing\n");
		*/		
